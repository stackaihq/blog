...
title: Dynamic OG image generation in Stackai blog posts
postSlug: dynamic-og-image-generation-in-Stackai-blog-posts
featured: false
draft: false
tags:
  - docs
  - release
ogImage: ""
description: New feature in Stackai v1.4.0, introducing dynamic OG image generation for blog posts.
---

New feature in Stackai v1.4.0, introducing dynamic OG image generation for blog posts.
...
Stackai already provided a way to add an OG image to a blog post. The author can specify the OG image in the frontmatter `ogImage`. Even when the author doesn't define the OG image in the frontmatter, the default OG image will be used as a fallback (in this case `public/Stackai-og.jpg`). But the problem is that the default OG image is static, which means every blog post that does not include an OG image in the frontmatter will always use the same default OG image despite each post title/content being different from others.
...
In Stackai v1.4.0, Vercel's [Satori](https://github.com/vercel/satori) package is used for dynamic OG image generation.
...
## Anatomy of Stackai dynamic OG image

Dynamic OG image of Stackai includes _the blog post title_, _author name_ and _site title_. Author name and site title will be retrieved via `SITE.author` and `SITE.title` of **"src/config.ts"** file. The title is generated from the blog post frontmatter `title`.
